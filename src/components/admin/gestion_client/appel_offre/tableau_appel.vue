...
const appel_offre = () =>{
  ...
  const tableau  = ref([
  { titre_de_ao: 'Directeur de la securité', date_publication: '12/09/2024', date_de_cloture: '12/12/2024', nombre_poste_disponilbe: 4,Annee_expeirience_requis: 5, diplome_requis: 'Master',domaine: 'informatique' , type_contrat: 'CDD',  specialite: 'Securité informatique' },
  { titre_de_ao: 'Directeur de la securité', date_publication: '12/09/2024', date_de_cloture: '12/12/2024', nombre_poste_disponilbe: 4,Annee_expeirience_requis: 5, diplome_requis: 'Master',domaine: 'informatique' , type_contrat: 'CDD',  specialite: 'Securité informatique' },
  { titre_de_ao: 'Directeur de la securité', date_publication: '12/09/2024', date_de_cloture: '12/12/2024', nombre_poste_disponilbe: 4,Annee_expeirience_requis: 5, diplome_requis: 'Master',domaine: 'informatique' , type_contrat: 'CDD',  specialite: 'Securité informatique' },
  { titre_de_ao: 'Directeur de la securité', date_publication: '12/09/2024', date_de_cloture: '12/12/2024', nombre_poste_disponilbe: 4,Annee_expeirience_requis: 5, diplome_requis: 'Master',domaine: 'informatique' , type_contrat: 'CDD',  specialite: 'Securité informatique' },
...
watch(recherche, nouvelle_valeur => {
      if (nouvelle_valeur !== undefined) { // Vérification si nouvelle_valeur est définie
        tableau.value = originalTableau.value.filter((utilisateur: { titre_de_ao: string; date_publication: string; date_de_cloture: string;Annee_expeirience_requis: { toString: () => string|string[]; }; diplome_requis: string; domaine: string; specialite: string; }) => {
          return (
            utilisateur.titre_de_ao.toLowerCase().includes(nouvelle_valeur.toLowerCase()) ||
            utilisateur.date_publication.toLowerCase().includes(nouvelle_valeur.toLowerCase()) ||
            utilisateur.date_de_cloture.toString().includes(nouvelle_valeur.toLowerCase()) || // Convertir en chaîne pour la comparaison
            utilisateur.Annee_expeirience_requis.toString().includes(nouvelle_valeur.toLowerCase()) ||
            utilisateur.diplome_requis.toLowerCase().includes(nouvelle_valeur.toLowerCase()) ||
            utilisateur.domaine.toLowerCase().includes(nouvelle_valeur.toLowerCase()) ||
            utilisateur.specialite.toLowerCase().includes(nouvelle_valeur.toLowerCase())
          );
        });
      }
    });
    ...
    <section class="formulaires" v-if="formulaire">
      ...
      <form class="formulaire" @submit.prevent="handleSubmit">
          <div class="slides">
            ...
            <div class="flex">
                <label>
                    <input required v-model="formData.date_cloture" placeholder="" type="date" class="input">
        ...
        <table>
          ...
          <tbody>
              <tr v-for="(ligne, index) in tableau" :key="index">
                  ...
                  <td class="">{{ligne.titre_de_ao}}</td>   
                  <td class="">{{ligne.date_publication}}</td>
                  <td class="">{{ligne.date_de_cloture}}</td> 
                  <td class="">{{ligne.Annee_expeirience_requis}}</td> 
                  <td class="">{{ligne.domaine}}</td>
                  <td class="">{{ligne.diplome_requis}}</td>                     
                  <td class="">{{ligne.specialite}}</td>
                  <td class="">{{ligne.nombre_poste_disponilbe}}</td>
